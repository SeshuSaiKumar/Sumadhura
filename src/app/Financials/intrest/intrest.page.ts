...
@Component({
  ...
  styleUrls: ['./intrest.page.scss'],
})
export class IntrestPage implements OnInit {
  ...
  async Invoices_view() {
    ...
    return CapacitorHttp.post(options).then(response => {
      ...
      if (response.data.responseCode == 200) {
        this.intrest_letters = [];
        this.intrest_letters = response.data.demandNote;
